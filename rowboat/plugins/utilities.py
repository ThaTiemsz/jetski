...
from rowboat.constants import (
    STATUS_EMOJI, BADGE_EMOJI, EMOJI_RE, USER_MENTION_RE, CDN_URL, CHANNEL_CATEGORY_EMOJI, TEXT_CHANNEL_EMOJI,
    VOICE_CHANNEL_EMOJI, ROLE_EMOJI, EMOJI_EMOJI, PREMIUM_GUILD_TIER_EMOJI, PREMIUM_GUILD_ICON_EMOJI,
)
...
def get_emoji_url(emoji):
    return CDN_URL.format('-'.join(str(char.encode("unicode_escape"), "utf-8")[2:].lstrip("0") for char in emoji))
...
class UtilitiesPlugin(Plugin):
    ...
    def emoji(self, event, emoji):
        if not EMOJI_RE.match(emoji):
            return event.msg.reply('Unknown emoji: `{}`'.format(S(emoji)))
        ...
        guild = self.bot.client.state.guilds.find_one(lambda v: eid in v.emojis)
        if guild:
    ...
    def get_emoji_url(self, emoji):
        name = '-'.join(str(char, "utf-8")[2:].lstrip("0") for char in emoji)
    ...
    def seen(self, event, user):
        ...
        event.msg.reply('I last saw {} {} ago (at {})'.format(
            user,
    ...
    def search(self, event, query):
        ...
        if len(users) == 1:
            if users[0].user_id in self.bot.client.state.users:
                return self.info(event, self.bot.client.state.users.get(users[0].user_id))

        return event.msg.reply('Found the following users for your query: ```{}```'.format(
           '\n'.join(map(lambda i:'{} ({})'.format(i, i.user_id), users[:25]))
        ))
    ...
    def server(self, event, guild_id=None):
        guild = self.bot.client.state.guilds.get(guild_id) if guild_id else event.guild
        if not guild:
        ...
        content_server.append('**Created:** {} ago ({})'.format(
            humanize.naturaldelta(datetime.utcnow() - created_at),
            created_at.isoformat(),
        ))
        content_server.append('**Members:** {:,}'.format(guild.member_count))
        if len(guild.features) > 0:
            content_server.append('**Features:** {}'.format(', '.join(guild.features) or 'none'))
        content_server.append('**Voice region:** {}'.format(guild.region))

        if not rdb.exists('gmm:{}'.format(guild.id)):
            self.bot.client.state.guilds[guild.id].inplace_update(self.client.api.guilds_get(guild.id), ignored=[
                'channels',
                'members',
                'voice_states',
                'presences',
            ])
            rdb.set('gmp:{}'.format(guild.id), self.bot.client.state.guilds[guild.id].max_presences)
            rdb.set('gmm:{}'.format(guild.id), self.bot.client.state.guilds[guild.id].max_members)

        content_server.append('**Max presences:** {:,}'.format(int(rdb.get('gmp:{}'.format(guild.id)))))
        content_server.append('**Max members:** {:,}'.format(int(rdb.get('gmm:{}'.format(guild.id)))))
        ...
        content_counts.append('<{}> {} channel categories'.format(CHANNEL_CATEGORY_EMOJI, count.get('category', 0)))
        content_counts.append('<{}> {} text channels'.format(TEXT_CHANNEL_EMOJI, count.get('text', 0)))
        content_counts.append('<{}> {} voice channels'.format(VOICE_CHANNEL_EMOJI, count.get('voice', 0)))
        embed.add_field(name='\u276F Counts', value='\n'.join(content_counts), inline=True)

        content_counts2 = []
        content_counts2.append('<{}> {} roles'.format(ROLE_EMOJI, len(guild.roles)))
        static_emojis = len(list(filter(lambda e: not guild.emojis.get(e).animated, guild.emojis)))
        animated_emojis = len(list(filter(lambda e: guild.emojis.get(e).animated, guild.emojis)))
        content_counts2.append('<{}> {}/{total} static emojis'.format(
            EMOJI_EMOJI,
            static_emojis,
            total=self.get_max_emoji_slots(guild))
        )
        content_counts2.append('<{}> {}/{total} animated emojis'.format(
            EMOJI_EMOJI,
            animated_emojis,
            total=self.get_max_emoji_slots(guild))
        )
        ...
        # Boosts
        content_boosts = []
        content_boosts.append('<{}> Level {}'.format(PREMIUM_GUILD_TIER_EMOJI[guild.premium_tier], int(guild.premium_tier)))
        # real_boost_count = len(filter(lambda y: guild.members.get(y).premium_since, guild.members))
        content_boosts.append('<{}> {} boosts'.format(
    ...
    def info(self, event, user=None):
        ...
        if isinstance(user, int):
            user_id = user
            user = self.bot.client.state.users.get(user)

        if user and not user_id:
            user = self.bot.client.state.users.get(user.id)
        ...
        if user.presence:
            ...
            game = user.presence.activity
            if game and game.name:
                activity = ['Playing', 'Stream', 'Listening to', 'Watching', 'Custom Status'][int(game.type or 0)]
                if not game.type:
                    activity = None
                if activity:
                    game_name = game.state if game.type == ActivityTypes.CUSTOM else game.name
                    content.append('**{}:** {}'.format(activity,
                       '[{}]({})'.format(game_name, game.url) if game.url else game_name
                    ))
        ...
        ).limit(1).order_by(Message.timestamp.desc())

        oldest_msg = Message.select(Message.timestamp).where(
            (Message.author_id == user.id) &
            (Message.guild_id == event.guild.id)
        ).limit(1).order_by(Message.timestamp.asc())
        ...
        if voice.value:
            ...
            content.append('\n**\u276F Voice**')
            content.append('**Sessions:** {:,}'.format(voice[0][1]))
            content.append('**Time:** {}'.format(humanize.naturaldelta(
                voice[0][2]
            )))
